...
const Catalog = () => {
    ...
    const Search = (event) => {
        setFilteredItems(JewelryData.filter(item => item.title.toLowerCase().includes(event.target.value.toLowerCase())));
    }
    ...
    const filterItems = () => {
        if (selectedFilters.length > 0) {
            const filteredData = JewelryData.filter((item) => selectedFilters.includes(item.category));
